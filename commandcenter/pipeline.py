...
def pipeline_total(rows) -> int:
    ...
    """
    # Build a mapping from deal id to its USD amount, ensuring each open deal is counted only once.
    ...
    for r in rows:
        ...
        deal_id = r.get("id")
        if deal_id is None:
            # Defensive: skip rows without an id (should not happen in normal data)
            continue
