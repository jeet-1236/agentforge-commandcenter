...
def pipeline_total(rows) -> int:
    ...
    """
    # Build a mapping from deal id to its USD amount for open deals, ensuring each deal contributes only once.
    deal_amounts = {r["id"]: int(r["amount_usd"]) for r in rows if is_open(r)}
