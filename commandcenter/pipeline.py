...
def pipeline_total(rows) -> int:
    ...
    """
    # Deduplicate by deal ID, keeping the (identical) amount_usd for each open deal.
    amounts_by_id: dict[str, int] = {}
    for row in rows:
        if is_open(row):
            amounts_by_id[row["id"]] = int(row["amount_usd"])
    return sum(amounts_by_id.values())
